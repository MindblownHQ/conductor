...
class Admin implements Hookable{
	...
	public function init(): void {
		...
		$routes = conductor()->routes()->get();
		$first_route  = $this->get_first_public_route( $routes );
		...
		$screen_id = \add_menu_page(
			...
			function() use ( $first_route ){
				Router::handle_callback( $first_route );
			},
			$svg_icon
		);
	...
	/**
	 * Return the first public route
	 */
	public function get_first_public_route( array $routes ): array {
		foreach( $routes as $route ){
			if( isset( $route['menu_label'] ) && $route['method'] == 'GET' && isset( $route['callback'] ) ){
				return $route;
			}
		}
